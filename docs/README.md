# Documentation Index

## Setup and Provider

- Existing local/setup guides: [setup](setup/README.md)
- Production credential map: [provider-setup/credential-map.md](provider-setup/credential-map.md)
- GitHub OAuth/App, Midtrans, PayPal, billing/currency, scanner, report/invoice:
  [provider-setup](provider-setup/)

## Deployment

- [Architecture](deployment/architecture.md)
- [VPS installation](deployment/vps-installation.md)
- [DNS](deployment/dns.md), [Nginx](deployment/nginx.md), [SSL](deployment/ssl-certbot.md)
- [Admin user](deployment/admin-user-setup.md) and [Admin Settings](deployment/admin-settings-setup.md)
- [Running operations](deployment/running-operations.md)
- [Backup/restore](deployment/backup-restore.md)
- [Troubleshooting](deployment/troubleshooting.md)
- [Security](deployment/security.md)
- [Post-deploy checklist](deployment/post-deploy-checklist.md)
- [Smoke test](deployment/smoke-test.md)

## Launch and Maintenance

- [Launch runbook](launch/launch-runbook.md)
- [Rollback](launch/rollback.md)
- [Maintenance](maintenance.md)

## Assurance

- [Security documentation](security/)
- [QA documentation](qa/)
- [Known limitations](known-limitations.md)
